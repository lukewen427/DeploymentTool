...
/**
 * This class performs the latest by index query
 * @author hugo
 */
public class LatestByIndexQueryEnactor extends DatasetQueryEnactor {

    public LatestByIndexQueryEnactor() {
    ...
    public JSONContainer performQuery() throws ConnexienceException {
        ...
        try {
            session = getSessionProvider().getSession();
            int size = DatasetsUtils.getJsonMultipleValueDataSize(session, (JsonMultipleValueItem)getItem());
            LatestByIndexQuery q = (LatestByIndexQuery)getQuery();
            if(size>0){
                int startPos = size - q.getRows();
                if(startPos<0){
                    startPos = 0;
                }
                return DatasetsUtils.getJsonMultipleValueData(session, (JsonMultipleValueItem)item, startPos , q.getRows(), q.getKeyArray());
            ...
            getSessionProvider().closeSession(session);
        }
    }    
}
