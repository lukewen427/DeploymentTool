...
/**
 * This class provides an interface to Macports for installing packages
 * @author hugo
 */
public class MacportsPackageManager implements PackageManager {
    ...
    public InstallResult installPackage(String packageName, String extraArguments, LibraryPreparationReport report) throws InstallerException {
        ...
        try {
            report.addMessage(LibraryPreparationReport.INFORMATION_MESSAGE, "Installing: " + packageName + " using Macports");
            int exitCode = runner.run("sudo /opt/local/bin/port install " + packageName + " " + extraArguments);
            if(exitCode==0){
                ...
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOADED_OK, packageName + " installed OK using Macports");
                return result;
            } else {
                // Command failed
                InstallResult result = new InstallResult(InstallResult.InstallStatus.INSTALL_FAILED, runner.sysOut(), runner.sysErr(), exitCode);
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOAD_FAILED, packageName + " installed FAILED using Macports: \n" + runner.sysErr() + "\n");
                return result;
            }   
            ...
            if(runner.isCommandStarted()){
                ...
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOAD_FAILED, packageName + " installed FAILED using Macports: \n" + runner.sysErr() + "\n");
                throw new InstallerException(result, "Error installing package", e);
                        
            } else {
                throw new InstallerException(null, "Error installing package", e);
            }
        }        
    }
    ...
    public boolean isInstalled(String packageName, LibraryPreparationReport report) throws InstallerException {
        CommandRunner runner = new CommandRunner();       
        try {
            report.addMessage(LibraryPreparationReport.INFORMATION_MESSAGE, "Checking status of package: " + packageName + " using Macports");
            int exitCode = runner.run("sudo /opt/local/bin/port installed " + packageName);
            if(exitCode==0){
                // Command ran, check output
                if(runner.sysOut().contains("The following ports are currently installed")){
                    report.addMessage(LibraryPreparationReport.INFORMATION_MESSAGE, packageName + " is installed");
                    return true;
                } else if(runner.sysOut().contains("None of the specified ports are installed")) {
            ...
            if(runner.isCommandStarted()){
                ...
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOAD_FAILED, "Could not start macports to check installation status of: " + packageName);
    ...
    public InstallResult updateInstalledPackages(LibraryPreparationReport report) throws InstallerException {
        CommandRunner runner = new CommandRunner();       
        try {
            int exitCode = runner.run("sudo /opt/local/bin/port selfupdate");
            if(exitCode==0){
                // Command ran, check output
                int exitCode2 = runner.run("sudo /opt/local/bin/port upgrade outdated");
                if(exitCode2==0){
                    InstallResult result = new InstallResult(InstallResult.InstallStatus.INSTALLED_OK, runner.sysOut(), runner.sysErr(), exitCode);
                    return result;
