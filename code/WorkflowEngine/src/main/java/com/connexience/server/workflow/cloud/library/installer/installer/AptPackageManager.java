...
public class AptPackageManager implements PackageManager {
    ...
    public InstallResult installPackage(String packageName, String extraArguments, LibraryPreparationReport report) throws InstallerException {
        ...
        try {
            report.addMessage(LibraryPreparationReport.INFORMATION_MESSAGE, "Installing: " + packageName + " using APT");
            int exitCode = runner.run("sudo /usr/bin/apt-get -q -y install " + packageName + " " + extraArguments);
            if(exitCode==0){
                ...
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOADED_OK, packageName + " installed OK using APT");
                return result;
            } else {
                // Command failed
                InstallResult result = new InstallResult(InstallResult.InstallStatus.INSTALL_FAILED, runner.sysOut(), runner.sysErr(), exitCode);
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOAD_FAILED, packageName + " installed FAILED using APT: \n" + runner.sysErr() + "\n");
                return result;
            }   
    ...
    public boolean isInstalled(String packageName, LibraryPreparationReport report) throws InstallerException {
        CommandRunner runner = new CommandRunner();       
        try {
            report.addMessage(LibraryPreparationReport.INFORMATION_MESSAGE, "Checking status of package: " + packageName + " using APT/dpkg");
            int exitCode = runner.run("sudo /usr/bin/dpkg-query -l " + packageName);
            if(exitCode==0){
                // Command ran, check output
            ...
            if(runner.isCommandStarted()){
                ...
                report.addMessage(LibraryPreparationReport.ITEM_DOWNLOAD_FAILED, "Could not start dpkg to check installation status of: " + packageName);
                throw new InstallerException(null, "Error checking package status", e);
            }
        }
    }
    ...
    public InstallResult updateInstalledPackages(LibraryPreparationReport report) throws InstallerException {
        CommandRunner runner = new CommandRunner();       
        try {
            int exitCode = runner.run("sudo /usr/bin/apt-get update");
            if(exitCode==0){
                // Command ran, check output
                int exitCode2 = runner.run("sudo /usr/bin/apt-get -y -q upgrade");
                if(exitCode2==0){
                    InstallResult result = new InstallResult(InstallResult.InstallStatus.INSTALLED_OK, runner.sysOut(), runner.sysErr(), exitCode);
                    return result;
